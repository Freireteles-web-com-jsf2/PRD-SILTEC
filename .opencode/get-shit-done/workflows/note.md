...
- **Global scope**: `D:/PRD-SILTEC/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/PRD-SILTEC/.opencode/notes/`)
...
2. Glob `D:/PRD-SILTEC/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (D:/PRD-SILTEC/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/PRD-SILTEC/.opencode/notes/` — works in any directory
