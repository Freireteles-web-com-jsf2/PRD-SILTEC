...
- **Global scope**: `D:/PRD-SILTEC/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/PRD-SILTEC/.claude/notes/`)
...
2. Glob `D:/PRD-SILTEC/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/PRD-SILTEC/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/PRD-SILTEC/.claude/notes/` — works in any directory
